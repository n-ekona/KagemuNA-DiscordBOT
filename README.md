...
| `/trend` | 全員 | 日ごとの**チャット数（やVC時間）の推移**を折れ線付き棒グラフで表示 |
...
### `/trend`（推移グラフ）のオプション
日ごとの件数を時系列の棒グラフ＋折れ線で表示します。
- `metric` … `メッセージ数`(既定) / `VCアクティブ時間`
- `period` … `直近7日` / `直近30日` / `今月` / `先月`。**未指定なら「週(7日)＋月(30日)」の両方**を一度に表示
- `from` / `to` … `YYYY-MM-DD`（`period` 未指定時の任意期間）
- `channel` … 対象チャンネルで絞り込み（任意）

使用例:
```
/trend                                  # 週と月の推移を両方表示
/trend metric:VCアクティブ時間 period:直近30日
/trend from:2026-06-01 to:2026-06-30 channel:#general
```
